...
// In-memory store for reminder settings (simulating persistence)
let globalReminderSettings = {
  enabled: true,
  channels: ['email', 'sms'],
  advanceReminders: ['24h', '1h']
};
...
const NotificationService = {
  async sendEmail(to: string, subject: string, body: string) {
    console.log(`[Resend.com] (MOCK) Sending email to: ${to}`);
    console.log(`Subject: ${subject}`);
    // console.log(`Body: ${body}`);
    return { id: `resend_${Math.random().toString(36).substr(2, 9)}` };
  },
  async sendSMS(to: string, message: string) {
    console.log(`[Twilio] (MOCK) Sending SMS to: ${to}`);
    console.log(`Message: ${message}`);
    return { id: `twilio_${Math.random().toString(36).substr(2, 9)}` };
  }
};
...
async function startServer() {
  ...
  app.post("/api/settings/reminders", (req, res) => {
    globalReminderSettings = { ...globalReminderSettings, ...req.body };
    console.log(`[Settings] Updated Reminder Preferences:`, globalReminderSettings);
    res.json({ success: true, settings: globalReminderSettings });
  });

  // Groq AI Clinical Brain - Streaming Implementation
  app.post("/api/ai-clinical-brain", async (req, res) => {
    ...
    try {
      ...
      const stream = await groq.chat.completions.create({
        messages: [
          {
            role: "system",
            content: "You are the 'Clinical Brain' for DentalOS. Treat all notes as HIPAA-compliant. Provide rapid, clinical analysis of patient symptoms, possible CDT codes, and treatment plans. Be professional, concise, and scientifically accurate."
          },
          ...messages
        ],
      ...
      res.status(500).write("Error processing AI request");
      res.end();
    }
  });

  // Demo requests log (to be connected to Firebase)
  app.post("/api/demo-request", async (req, res) => {
    const { name, clinicName, email, phone, preferredDate } = req.body;
    console.log(`[Demo Request Received] ${name} from ${clinicName} (${email}) requested demo for ${preferredDate}`);
    
    // In a final production flow, this would write to Firebase
  ...
  app.post("/api/book-appointment", async (req, res) => {
    ...
    if (globalReminderSettings.enabled) {
      ...
      console.log(`[Automation] Scheduling reminders for ${name}:`);
      globalReminderSettings.advanceReminders.forEach(time => {
        globalReminderSettings.channels.forEach(channel => {
          console.log(` - ${channel.toUpperCase()} reminder scheduled for ${time} before appt.`);
        });
      });
    }
