...
import { Users, Search, Filter, Phone, Mail, MoreHorizontal, AlertCircle, CheckCircle, ArrowLeft, History, FileText, Activity, Calendar as CalendarIcon, ClipboardList, CreditCard, LayoutList, ChevronLeft, ChevronRight, Camera, Upload, Loader2, X } from 'lucide-react';
...
export default function PatientTriage() {
  ...
  const filteredPatients = patients.filter(p => {
    const matchesUrgency = filter === 'all' ? true : p.urgency === filter;
    const matchesSearch = p.name.toLowerCase().includes(triageSearch.toLowerCase()) ||
                          p.email.toLowerCase().includes(triageSearch.toLowerCase()) ||
                          p.phone.includes(triageSearch);
          ...
          >
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
              <div>
                ...
                <p className="text-secondary mt-1 text-sm">Intelligent categorization of incoming patient requests.</p>
              </div>
              <div className="flex flex-wrap lg:flex-nowrap items-center gap-3">
                <div className="flex flex-wrap gap-1.5">
                  {['all', 'emergency', 'urgent', 'routine'].map((f) => (
                    <button
                      ...
                      onClick={() => setFilter(f)}
                      className={cn(
                        "px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all border cursor-pointer",
                        filter === f 
                ...
                <div className="flex bg-card border border-border rounded-xl p-1 shrink-0">
                  ...
                  <button 
                    ...
                    className={cn(
                      "p-1.5 rounded-lg transition-all cursor-pointer relative",
            ...
            {viewMode === 'list' ? (
              <div className="glass-card overflow-hidden">
                <div className="p-4 border-b border-border flex items-center justify-between bg-card/50">
                  <div className="flex items-center gap-3 px-3 py-1.5 bg-background rounded-xl w-full sm:w-1/2 md:w-1/3 border border-border">
                    ...
                    />
                    {triageSearch && (
                      <button onClick={() => setTriageSearch('')} className="p-0.5 text-secondary hover:text-white">
                ...
                <div className="block md:hidden divide-y divide-border">
                  {filteredPatients.map((patient) => (
                    <div key={patient.id} className="p-4 space-y-3 bg-white/[0.01]">
                      <div className="flex justify-between items-start">
                        <div 
                          className="cursor-pointer group/name"
                          onClick={() => setSelectedPatientId(patient.id)}
                        >
                          <h4 className="font-semibold text-primary group-hover/name:text-accent group-hover/name:underline underline-offset-2">{patient.name}</h4>
                          <span className="text-[9px] text-secondary font-mono bg-white/5 border border-white/5 px-1.5 py-0.5 rounded uppercase mt-1 inline-block">ID: {patient.id}</span>
                        </div>
                        <span className={cn(
                          "px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-widest border shrink-0",
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[11px] text-secondary">
                        <div className="space-y-1">
                          <p className="font-bold text-[8px] text-secondary/40 uppercase tracking-wider">Contact</p>
                          <a href={`tel:${patient.phone}`} className="flex items-center gap-1.5 hover:text-accent font-medium truncate">
                            <Phone className="w-3 h-3 text-accent" />
                            {patient.phone}
                          </a>
                          <a href={`mailto:${patient.email}`} className="flex items-center gap-1.5 hover:text-accent font-medium truncate">
                            <Mail className="w-3 h-3 text-accent" />
                            {patient.email}
                          </a>
                        </div>
                        <div className="space-y-1 text-right">
                          <p className="font-bold text-[8px] text-secondary/40 uppercase tracking-wider">Registered</p>
                          <p className="font-mono mt-1 text-primary">{new Date(patient.createdAt).toLocaleDateString()}</p>
                          <button 
                            onClick={() => setSelectedPatientId(patient.id)}
                            className="text-[10px] text-accent font-black hover:underline mt-1 block w-full text-right uppercase tracking-widest"
                          >
                            Clinical Chart ➜
                          </button>
                        </div>
